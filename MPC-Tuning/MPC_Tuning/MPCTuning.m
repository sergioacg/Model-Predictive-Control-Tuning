function [delta, lambda, Np, Nun, Fob] = MPCTuning(mpcobj, Sp, linear, w, fi, Pref, varargin)
...
% - fi: iteration number
% - Pref: transfer function reference
%
% Optional input arguments for non-linear systems:
%
% [delta, lambda, Np, Nun] = MPCTuning(mpcobj, Sp, linear, w, fi, Pref, nbp) uses `nbp` as
% the number of bits for the prediction horizon (default: 8 bits = 255 as a maximum prediction)
%
% [delta, lambda, Np, Nun] = MPCTuning(mpcobj, Sp, linear, w, fi, Pref, nbp, nbc) uses `nbc` as
% the number of bits for the control horizon (default: 4 bits = 15 as a maximum prediction)
%
% [delta, lambda, Np, Nun] = MPCTuning(mpcobj, Sp, linear, w, fi, Pref, nbp, nbc, nlmodel, init) sets 
...
%
% [delta, lambda, Np, Nun] = MPCTuning(mpcobj, Sp, linear, w, fi, Pref, nbp, nbc, nlmodel, init, optimset) sets 
...
if ni ==0+6
     nbp=8;nbc=4;
 elseif ni == 1+6
    nbp=varargin{1};nbc=4;
 elseif ni == 2+6
    nbp=varargin{1};nbc=varargin{2};
 elseif ni == 3+6
    error('When you place the nonlinear model, you need to append the initial condition of the model.')
 elseif ni == 4+6
    nbp=varargin{1};nbc=varargin{2};
    model=varargin{3};init=varargin{4};
 elseif ni == 5+6
    nbp=varargin{1};nbc=varargin{2};
    model=varargin{3};init=varargin{4};
    options  = varargin{5}; % Update options with user input
end
...
Par.Yref=Yref';     % Reference trajectory response
Par.Xsp=Xsp;        % Unitary set-point
